/**
 * MS. PAC-MAN — 1982 Midway Arcade Classic (BUILD V2)
...
(() => {
  ...
  // ── Audio (Midway Ms. Pac-Man Web Audio) — BUILD V2 ───────────────────────
  ...
  function showOV(title, sub, cls) {
    ...
    if (home) {
      $hint.textContent = "BUILD V2 — PRESS SPACE OR TAP TO START";
      if ($ctrl) {
        $ctrl.innerHTML =
          "ARROWS / WASD / SWIPE / D-PAD — MOVE<br />" +
          "SPACE OR PAUSE — START / PAUSE<br />" +
          "M OR MUTE — SOUND — MUST SEE BUILD V2";
      }
    }
  ...
  // ── Draw (BUILD V2 — fluent mazes + rounded cabinet sprites) ─────────────
  function wallAt(c, r) {
  ...
  function drawMaze() {
    ...
    // Soft color wash in thick wall blocks so boards read as different colors
    if (!flash) {
    ...
    for (let r = 0; r < ROWS; r++) {
      for (let c = 0; c < COLS; c++) {
        ...
        if (v === DOT) {
          ...
          // Smooth pulsing energizer disc
          const pulse = 0.72 + 0.28 * Math.sin(time / 140);
          const rad = 6.2 * S * pulse;
          ctx.fillStyle = dotCol;
          ctx.beginPath();
  ...
  function drawMsPacBody(cctx, rad, gap) {
    ...
    // Red bow (two loops + knot) — signature Ms. Pac
    cctx.fillStyle = "#ff0044";
    cctx.beginPath();
    cctx.ellipse(-rad * 0.18, -rad * 0.78, rad * 0.3, rad * 0.2, -0.45, 0, Math.PI * 2);
    cctx.fill();
    cctx.beginPath();
    cctx.ellipse(rad * 0.26, -rad * 0.72, rad * 0.3, rad * 0.2, 0.4, 0, Math.PI * 2);
    cctx.fill();
    cctx.beginPath();
    cctx.arc(rad * 0.04, -rad * 0.7, rad * 0.13, 0, Math.PI * 2);
    cctx.fill();
    // Bow highlight
    cctx.fillStyle = "#ff6699";
    cctx.beginPath();
    cctx.ellipse(-rad * 0.22, -rad * 0.82, rad * 0.1, rad * 0.06, -0.45, 0, Math.PI * 2);
    cctx.ellipse(rad * 0.3, -rad * 0.76, rad * 0.1, rad * 0.06, 0.4, 0, Math.PI * 2);
    cctx.fill();
    ...
    // Beauty mark
    cctx.beginPath();
    cctx.arc(rad * 0.4, rad * 0.14, rad * 0.065, 0, Math.PI * 2);
    cctx.fill();

    // Soft lipstick along mouth edge
    if (gap >= 0.12) {
      cctx.strokeStyle = "#ff3366";
      cctx.lineWidth = 1.5 * S;
      cctx.lineCap = "round";
      cctx.beginPath();
      cctx.arc(0, 0, rad * 0.94, gap + 0.04, Math.PI * 2 - gap - 0.04);
      cctx.stroke();
    }
  }
  ...
  function ghostBodyPath(cctx, x, y, R, phase) {
    // Rounded dome head + soft animated scalloped skirt
    const top = y - R * 0.98;
    const bot = y + R * 1.02;
    const left = x - R;
    const right = x + R;
  ...
  /** Rounded dome + scallops — BUILD V2 */
  ...
  console.log("[MS. PAC-MAN BUILD V2] fluent mazes + round sprites + solid music");
  requestAnimationFrame(tick);
})();
